...
class APIV1UserPasswordsTest < ActionDispatch::IntegrationTest
  test "#create responds with 400 when params are missing" do
    ...
    post(api_v1_helper.user__passwords_url, params:)
  ...
  test "#create responds with 200 when the email is not found" do
    ...
    post(api_v1_helper.user__passwords_url, params: { user: { email: } })
  ...
  test "#create sends reset password email and responds with 200" do
    ...
    emails = capture_emails do
      post(api_v1_helper.user__passwords_url, params: { user: { email: user.email } })
    end
  ...
  test "#update responds with 422 when token is invalid" do
    ...
    put(api_v1_helper.user__password_url(token, format: :json), params:)
