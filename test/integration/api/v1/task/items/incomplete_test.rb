...
class APIV1TaskItemsIncompleteTest < ActionDispatch::IntegrationTest
  test "#update responds with 401 when API token is invalid" do
    ...
    put(api_v1_helper.incomplete_task__item_url(member!(user).inbox, task), headers:)
  ...
  test "#update responds with 404 when task list is not found" do
    ...
    url = api_v1_helper.incomplete_task__item_url(Account::Task::List.maximum(:id) + 1, task.id)
  ...
  test "#update responds with 404 when task is not found" do
    ...
    url = api_v1_helper.incomplete_task__item_url(member!(user).inbox, Account::Task::Item.maximum(:id) + 1)
  ...
  test "#update responds with 404 when task list belongs to another user" do
    ...
    put(
      api_v1_helper.incomplete_task__item_url(task.list, task),
  ...
  test "#update responds with 200 when task is marked as completed" do
    ...
    assert_changes -> { task.reload.completed_at } do
      put(
        api_v1_helper.incomplete_task__item_url(member!(user).inbox, task),
        headers: api_v1_helper.authorization_header(user)
      )
    end
